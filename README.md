...
### Steps to Install:
 ...
 - point the folder-picker pop-up at the directory in this repo `/mpEZTrack/chromeExtension`; you'll see the extension installed
...
## single page applications (SPAs) 🧖 <div id="spa"></div>
in it's default mode, `mpEZTrack` queries the DOM for trackable elements _only once_ ... when the page is fully loaded. this is not feasible for client-rendered single page applications (SPAs) like react, vue, angular, etc... which construct the DOM reactively, as an end-user navigates through an application.

it is still possible to use `mpEZTrack` on such "client-rendered" applications by passing the **`{ spa : true} `** option to `.init()` like this:

```javascript
mpEZTrack.init('YOUR-PROJECT-TOKEN', {spa: true})
```
...
SPA mode is an appropriate choice for any apps which construct their UI components _dynamically_ and _not_ by ` DOMContentLoaded`. 


## user profiles 👥 <div  id="profiles"></div>
...
## security + sensitive fields 🔓 <div  id="security"></div>
